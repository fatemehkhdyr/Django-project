    ...
    def form_valid(self, form):
        ...
        if self.comment.parent :
            ...
            if parent_email in [author_email, user_email]:
                parent_email = False
        if author_email:
            email = EmailMessage('ارسال یک دیدگاه برای شما',
                               ' دیدگاهی برای مقاله ی {}ارسال شد برای دیدن این مقاله به لینک مقابل مراجعه کنید : {}{}'.format(article.title, current_site,reverse('blog:detail', kwargs={'slug':article.slug})),
                                  to=[author_email])
            email.send()

        if user_email:
            email = EmailMessage('ارسال یک دیدگاه برای شما',
                              'دیدگاه شما در مقاله ثبت شد.',
                                  to=[user_email])
            email.send()

        if parent_email:
            email = EmailMessage('ارسال یک دیدگاه برای شما',
                               ' پاسخی برای دیدگاه شما در مقاله ی {}ارسال شد برای دیدن این پاسخ به لینک مقابل مراجعه کنید : {}{}'.format(article.title, current_site,reverse('blog:detail', kwargs={'slug':article.slug})),
                                  to=[parent_email])
            email.send()
